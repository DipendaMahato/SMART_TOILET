...
export function UrineQualityResult({ tdsValue, turbidityValue }: UrineQualityResultProps) {
  ...
  if (tdsStatus === 'Slightly Abnormal') {
    conclusionParts.push('TDS level is slightly elevated.');
  } else if (tdsStatus === 'Abnormal') {
    conclusionParts.push('High TDS levels were detected, which may suggest dehydration or other health issues.');
  }

  if (turbidityStatus === 'Slightly Abnormal') {
    conclusionParts.push('The sample appears slightly cloudy.');
  } else if (turbidityStatus === 'Abnormal') {
    conclusionParts.push('The sample is cloudy, which can sometimes indicate a urinary tract infection.');
  }
  ...
  if (conclusionParts.length > 0) {
    ...
    if (tdsStatus === 'Abnormal' || turbidityStatus === 'Abnormal') {
      ...
      conclusion += ' Further evaluation is recommended.';
    } else {
      overallStatus = 'Slightly Abnormal';
      conclusion += ' No immediate concern is indicated, but monitoring is advised.';
    }
  } else {
